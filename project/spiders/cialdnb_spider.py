...
urls_doc = input(
    'Filename of websites list with extension (Ex: websites.txt): ')
...
class CialdnbSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        clear_phones = []
        for number in page:
            ph = number.re(r'\(\d{1,5}\)[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}')
            telefones.append(ph)

        for i in list(range(0, len(telefones))):
            ks = len(telefones[i])
            for k in list(range(0, ks)):
                l = telefones[i][k].replace('(', '').replace(
                    ')', '').replace('-', ' ').replace('.', ' ')
        ...
        for numb in page:
            ph2 = page.re(
                r'\+[\s]??\d{1,3}[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}|\+[\s]??\d{1,3}[-\.\s]??\(\d{1,5}\)[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}')
        ...
        for i in list(range(0, len(telefones2))):
            ks = len(telefones2[i])
            for k in list(range(0, ks)):
                l = telefones2[i][k].replace('(', '').replace(
                    ')', '').replace('-', ' ').replace('.', ' ')
        ...
        for numb in page:
            ph2 = page.re(
                r'\+[\s]??\d{1,3}[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}|\+[\s]??\d{1,3}[-\.\s]??\(\d{1,5}\)[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}[-\.\s]??\d{1,5}')
            telefones3.append(ph2)

        for i in list(range(0, len(telefones3))):
            ks = len(telefones3[i])
            for k in list(range(0, ks)):
                l = telefones3[i][k].replace('(', '').replace(
                    ')', '').replace('-', ' ').replace('.', ' ')
                clear_phones3.append(l)
        non_duplicate_phones3 = list(dict.fromkeys(clear_phones3))

        final_phones = non_duplicate_phones.copy() + non_duplicate_phones2.copy()
        ...
        for g in list(range(0, len(nova_lista))):
            ...
            for i in nova_lista1:
                n1 = i
                n2 = k
                n1limpo = n1.replace('(', '').replace(')', '').replace(
                    '-', ' ').replace(' ', '').replace('+', '').replace('.', '')
                n2limpo = n2.replace('(', '').replace(')', '').replace(
                    '-', ' ').replace(' ', '').replace('+', '').replace('.', '')
        ...
        url_list = []
        case_list = []
        homepage = response.url.split(
            '/')[0] + "//" + response.url.split('/')[2]
            ...
            for tag_a in response.xpath('//a'):
                ...
                if a_href[:6] == str("index.") or a_href == homepage:
                    for tag_img in tag_a.xpath('.//img'):
                        ...
                        tag_class = str(tag_img.xpath(
                            '@class').extract()).lower().strip()
                        title = str(tag_img.xpath(
                            '@title').extract()).lower().strip()
                        alt = str(tag_img.xpath(
                            '@alt').extract()).lower().strip()
                        ...
                        if img_ext in ext_list or tag_class.find("logo") > 0 or title.find("logo") > 0 or \
                            ...
                            if img_url[0] == '/':
                                ...
                                img_url = str(tag_img.xpath(
                                    '@src').extract()).replace("['", "").replace("']", "")

                            img_url_list.append(img_url)

        yield {
            'Logo': img_url_list,
            'Phone Numbers': final_phones,
